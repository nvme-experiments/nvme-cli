		...
		struct plugin *plugin)
{
	...
	err = nvme_set_features(l, 0, cfg.feature_id, cfg.save, cfg.value, 0, 0, 0,
						 0, NULL, 0, &result);
	if (err < 0)
		perror("set-feature");
	if (!err)
		...
		struct plugin *plugin)
{
	...
	err = nvme_set_features(l, 0, cfg.feature_id, false, cfg.value, 0, 0, 0,
						 0, NULL, 0, &result);
	if (err < 0)
		perror("set-feature");
	if (!err)
		...
		struct plugin *plugin)
{
	...
	err = nvme_get_log_simple(l, GLP_ID_VU_GET_HIGH_LATENCY_LOG, &buf, sizeof(buf));

	while (1) {
		if (!glp_high_latency(fdi, buf, LOG_PAGE_SIZE, DO_PRINT_FLAG))
			break;
		err = nvme_get_log_simple(l, GLP_ID_VU_GET_HIGH_LATENCY_LOG,
					  &buf, sizeof(buf));
		if (err) {
			nvme_show_status(err);
			break;
...
static int mb_lat_stats_log_print(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log_simple(l, cfg.write ? 0xc2 : 0xc1, &stats, sizeof(stats));
	if (!err)
...
static int mb_set_lat_stats(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	...
	case True:
	case False:
		err = nvme_set_features(l, nsid, fid, save, option, cdw12, 0, 0,
...
static int mb_get_smart_log_add(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log_simple(l, LID_SMART_LOG_ADD, &log, sizeof(struct smart_log_add));
	if (!err) {
		if (!cfg.raw_binary)
...
static int mb_set_latency_feature(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_set_features(l, 0, FID_LATENCY_FEATURE, 0, 0 | cfg.perf_monitor,
						 0 | cfg.cmd_mask,
...
static int mb_get_latency_stats(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log_simple(l, LID_LATENCY_STATISTICS, &log, sizeof(struct latency_stats));
	if (!err) {
		if (!cfg.raw_binary)
				    ...
				    struct plugin *plugin)
{
	...
	err = nvme_get_log_simple(l, LID_PERFORMANCE_STATISTICS, &log, xfer_size);
	if (!err) {
		if (!cfg.raw_binary)
