...
static int fdp_stats(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log_fdp_stats(l, cfg.egid, 0, &stats, sizeof(stats));
	if (err) {
		nvme_show_status(err);
		return err;
	}
...
static int fdp_status(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_fdp_reclaim_unit_handle_status(l, cfg.namespace_id, &hdr,
						  sizeof(hdr), NULL);
	if (err) {
		nvme_show_status(err);
		return err;
	...
	err = nvme_fdp_reclaim_unit_handle_status(l, cfg.namespace_id, buf,
						  len, NULL);
	if (err) {
		nvme_show_status(err);
		return err;
...
static int fdp_set_events(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_set_features(l, cfg.nsid, NVME_FEAT_FID_FDP_EVENTS, cfg.sv,
