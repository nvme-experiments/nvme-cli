...
{
	return nvme_get_features(lt->link, 0, LATENCY_TRACKING_FID, 0, 0,
						  lt->uuid_index, NULL,
...
static int latency_tracking_enable(struct latency_tracker *lt)
{
	...
	err = nvme_set_features(lt->link, 0, LATENCY_TRACKING_FID, 0,
						 lt->cfg.enable, 0, 0, 0, lt->uuid_index, NULL,
