...
static int id_ctrl(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_zns_identify_ctrl(l, &ctrl);
	if (!err)
...
static int id_ns(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_identify_ns(l, cfg.namespace_id, &id_ns);
	if (err) {
		nvme_show_status(err);
		return err;
	}

	err = nvme_zns_identify_ns(l, cfg.namespace_id, &ns);
	if (!err)
	...
	const char *desc, enum nvme_zns_send_action zsa)
{
	...
	err = nvme_zns_mgmt_send(l, cfg.namespace_id, cfg.zslba, zsa, cfg.select_all, 0, NULL, 0,
				 &result);
	if (!err) {
...
static int get_zdes_bytes(nvme_link_t l, __u32 nsid)
{
	struct nvme_zns_id_ns ns;
	struct nvme_id_ns id_ns;
	__u8 lbaf;
	...
	err = nvme_identify_ns(l, nsid, &id_ns);
	if (err > 0) {
		nvme_show_status(err);
		return -1;
	} else if (err < 0) {
	...
	err = nvme_zns_identify_ns(l, nsid,  &ns);
	if (err > 0) {
		nvme_show_status(err);
		return -1;
	} else if (err < 0) {
...
static int zone_mgmt_send(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_zns_mgmt_send(l, cfg.namespace_id, cfg.zslba, cfg.zsa, cfg.select_all, cfg.zsaso,
				 buf, cfg.data_len, NULL);
	if (!err)
...
static int open_zone(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_zns_mgmt_send(l, cfg.namespace_id, cfg.zslba, NVME_ZNS_ZSA_OPEN, cfg.select_all,
				 cfg.zrwaa, NULL, 0, NULL);
	if (!err)
...
static int set_zone_desc(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	data_len = get_zdes_bytes(l, cfg.namespace_id);
	...
	err = nvme_zns_mgmt_send(l, cfg.namespace_id, cfg.zslba, NVME_ZNS_ZSA_SET_DESC_EXT, 0,
				 cfg.zrwaa, buf, data_len, NULL);
	if (!err)
...
static int zrwa_flush_zone(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_zns_mgmt_send(l, cfg.namespace_id, cfg.lba, NVME_ZNS_ZSA_ZRWA_FLUSH, 0, 0, NULL,
				 0, NULL);
	if (!err)
...
static int zone_mgmt_recv(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_zns_mgmt_recv(l, cfg.namespace_id, cfg.zslba, cfg.zra, cfg.zrasf, cfg.partial,
				 data, cfg.data_len, NULL);
	if (!err)
...
static int report_zones(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	if (cfg.extended) {
		zdes = get_zdes_bytes(l, cfg.namespace_id);
		if (zdes < 0)
			return zdes;
	}
	...
	err = nvme_zns_identify_ns(l, cfg.namespace_id, &id_zns);
	if (!err) {
...
static int changed_zone_list(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_get_log_zns_changed_zones(l, cfg.namespace_id,
					     cfg.rae, &log);
	if (!err)
