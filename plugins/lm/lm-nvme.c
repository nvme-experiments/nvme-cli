...
static int lm_create_cdq(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	...
	err = nvme_lm_cdq(l, NVME_LM_SEL_CREATE_CDQ, NVME_SET(cfg.qt, LM_QT), cfg.cntlid,
			  cfg.sz, queue, &cdqid, NULL);
	if (err < 0)
...
static int lm_track_send(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	...
	err = nvme_lm_track_send(l, cfg.sel, cfg.mos, cfg.cdqid, NULL);
	if (err < 0)
...
static int lm_migration_send(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	...
	err = nvme_lm_migration_send(l, cfg.sel, NVME_SET(cfg.seqind, LM_SEQIND),
				     cfg.cntlid, cfg.stype, cfg.dudmq,
				     0, cfg.csuuidi, cfg.offset,
				     cfg.uidx, data, cfg.numd << 2,
				     NULL);
	if (err < 0)
...
static int lm_set_cdq(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	...
	err = nvme_set_features(l, 0, lm_cdq_feature_id, 0, cfg.cdqid |
