...
static int io_mgmt_send(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_io_mgmt_send(l, cfg.namespace_id, cfg.mo, cfg.mos,
...
static int sanitize_cmd(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_sanitize_nvm(l, cfg.sanact, cfg.ause, cfg.owpass,
				cfg.oipbp, cfg.no_dealloc,
...
static int copy_cmd(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_copy(l, cfg.namespace_id, cfg.sdlba, nr, cfg.format,
			cfg.prinfor, cfg.prinfow, 0, cfg.dtype, false,
			false, cfg.fua, cfg.lr, 0, cfg.dspec,
			false, 0, 0, cfg.ilbrt, 0,
			cfg.lbat, cfg.lbatm,
			copy->f0,
			NULL);
	if (err < 0)
		...
		struct plugin *plugin)
{
	...
	err = nvme_get_lba_status(l, cfg.namespace_id, cfg.slba,
				  cfg.mndw, cfg.atype, cfg.rl, buf,
				  NULL);
	if (!err)
...
static int dir_receive(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_directive_recv(l, cfg.namespace_id, cfg.doper, cfg.dtype,
				  cfg.dspec, dw12, buf,
				  cfg.data_len, &result);
	if (!err)
