...
static int io_mgmt_recv(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_io_mgmt_recv(l, cfg.namespace_id, cfg.mos, cfg.mo, buf, cfg.data_len, NULL);
	if (!err) {
...
static int list(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	r = nvme_create_root(stdout, log_level);
	if (!r) {
		nvme_show_error("Failed to create topology root");
		return -ENOMEM;
	}
	err = nvme_scan_topology(r, NULL, NULL);
	if (err < 0) {
		nvme_show_error("Failed to scan topology: %d", nvme_strerror(-err));
		return err;
	}
...
static int write_uncor(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_write_uncorrectable(l, cfg.namespace_id, cfg.start_block, cfg.block_count,
				       cfg.dtype << 4, cfg.dspec, NULL);
	if (err < 0)
...
static int dsm(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_dsm(l, cfg.namespace_id, cfg.cdw11, dsm, nr, NULL);
	if (err < 0)
...
static int resv_acquire(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_resv_acquire(l, cfg.namespace_id, cfg.crkey, cfg.prkey, cfg.rtype, cfg.racqa,
				!!cfg.iekey, NULL);
	if (err < 0)
...
static int resv_register(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_resv_register(l, cfg.namespace_id, cfg.crkey, cfg.nrkey, cfg.rrega, cfg.cptpl,
				 !!cfg.iekey, NULL);
	if (err < 0)
...
static int resv_release(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_resv_release(l, cfg.namespace_id, cfg.crkey, cfg.rtype, cfg.rrela, !!cfg.iekey,
				NULL);
	if (err < 0)
...
static int resv_report(int argc, char **argv, struct command *cmd, struct plugin *plugin)
{
	...
	err = nvme_resv_report(l, cfg.namespace_id, cfg.eds, status, size, NULL);
	if (!err)
...
static int submit_io(int opcode, char *command, const char *desc, int argc, char **argv)
{
	...
	err = nvme_io(l, opcode, cfg.namespace_id, cfg.start_block, cfg.storage_tag,
		      (__u32)cfg.ref_tag, nblocks, control, cfg.app_tag, cfg.app_tag_mask,
		      cfg.dspec, cfg.dsmgmt, cfg.ref_tag, sts, pif, buffer, buffer_size, mbuffer,
		      mbuffer_size, NULL);
...
static int append_keyfile(const char *keyring, long id, const char *keyfile)
{
	...
	err = nvme_export_tls_key_versioned(ver, hmac,
					    key_data, key_len,
					    &exported_key);
	if (err) {
...
static int gen_tls_key(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	...
	nvme_export_tls_key(raw_secret, key_len, &encoded_key);
	printf("%s\n", encoded_key);
			   ...
			   char *desc, int desc_len, void *data)
{
	...
	if (!nvme_read_key(keyring_id, key_id, &key_len, &key_data))
		return;

	if (sscanf(desc, "NVMe%01d%c%02d %*s", &ver, &type, &hmac) != 3)
		return;

	if (!nvme_export_tls_key_versioned(ver, hmac,
					   key_data, key_len,
					   &encoded_key))
		return;
	fprintf(fd, "%s %s\n", desc, encoded_key);
